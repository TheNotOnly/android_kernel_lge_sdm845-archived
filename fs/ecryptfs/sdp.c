			...
			struct ecryptfs_key_record *key_rec)
{
	...
	if (rc < 0 || rc == SDP_NOT_EXIST) {
...
#ifdef CONFIG_ECRYPT_FS_KEY_ENCRYPTION_GCM
		rc = virt_to_scatterlist(crypt_stat->key, crypt_stat->key_size, src_sg, 2);
		...
		rc = virt_to_scatterlist(key_rec->enc_key, crypt_stat->key_size + DEFAULT_GCM_AUTHSIZE, dst_sg, 2);
		if (rc < 1 || rc > 2) {
			SDP_LOGE("Error generating scatterlist "
					"for crypt_stat encrypted session key; "
					"expected rc = 1; got rc = [%d]. "
					"key_rec->enc_key_size + DEFAULT_GCM_AUTHSIZE = [%zd]\n", rc,
					key_rec->enc_key_size + DEFAULT_GCM_AUTHSIZE);
			rc = -ENOMEM;
			goto out;
		}
		rc = ecryptfs_gcm_encrypt(1, src_sg, dst_sg, crypt_stat->key_size,
			 sym_key->data, sym_key->len, crypt_stat->gcm_iv);
#else
				  ...
				  struct ecryptfs_crypt_stat *crypt_stat)
{
	...
	if (!(crypt_stat->flags & ECRYPTFS_SDP_SENSITIVE)) {
...
#ifdef CONFIG_ECRYPT_FS_KEY_ENCRYPTION_GCM
		rc = virt_to_scatterlist(auth_tok->session_key.encrypted_key, auth_tok->session_key.encrypted_key_size + DEFAULT_GCM_AUTHSIZE, src_sg, 2);
		if (rc < 1 || rc > 2) {
		...
		rc = virt_to_scatterlist(auth_tok->session_key.decrypted_key, auth_tok->session_key.decrypted_key_size, dst_sg, 2);
		if (rc < 1 || rc > 2) {
			printk(KERN_ERR "Internal error whilst attempting to convert "
		...
		rc = ecryptfs_gcm_encrypt(0, src_sg, dst_sg,
				auth_tok->session_key.encrypted_key_size + DEFAULT_GCM_AUTHSIZE,
				sym_key.data, sym_key.len, crypt_stat->gcm_iv);
#else
	...
	if (rc < 0 || rc == SDP_NOT_EXIST) {
		...
		rc = virt_to_scatterlist(auth_tok->session_key.encrypted_key,
				auth_tok->session_key.encrypted_key_size + DEFAULT_GCM_AUTHSIZE, src_sg, 2);
		if (rc < 1 || rc > 2) {
			printk(KERN_ERR "Internal error whilst attempting to convert "
				"auth_tok->session_key.encrypted_key to scatterlist; "
				"expected rc = 1; got rc = [%d]. "
		...
		rc = virt_to_scatterlist(auth_tok->session_key.decrypted_key,
				auth_tok->session_key.decrypted_key_size, dst_sg, 2);
		if (rc < 1 || rc > 2) {
			printk(KERN_ERR "Internal error whilst attempting to convert "
				"auth_tok->session_key.decrypted_key to scatterlist; "
				"expected rc = 1; got rc = [%d]\n", rc);
			...
			struct ecryptfs_key_record *key_rec, size_t *packet_size)
{
	...
	key_rec->enc_key_size = auth_tok->session_key.encrypted_key_size;
	encrypted_session_key_valid = 0;
#endif /* CONFIG_ECRYPT_FS_KEY_ENCRYPTION_GCM */
	...
	if (crypt_stat->flags & ECRYPTFS_SDP_SENSITIVE) {
		...
		if (rc < 1 || rc > 2) {
			ecryptfs_printk(KERN_ERR, "Error generating scatterlist "
					"for crypt_stat session key; expected rc = 1; "
					"got rc = [%d]. key_rec->enc_key_size = [%zd]\n",
					rc, crypt_stat->key_size);
			rc = -ENOMEM;
			goto out;
		}
		rc = virt_to_scatterlist(key_rec->enc_key, crypt_stat->key_size + authsize, dst_sg, 2);
		if (rc < 1 || rc > 2) {
			SDP_LOGE("Error generating scatterlist "
					"for crypt_stat encrypted session key; "
					"expected rc = 1; got rc = [%d]. "
					"key_rec->enc_key_size + DEFAULT_GCM_AUTHSIZE = [%zd]\n", rc,
					crypt_stat->key_size + DEFAULT_GCM_AUTHSIZE);
			rc = -ENOMEM;
			goto out;
		}

		rc = ecryptfs_gcm_encrypt(1, src_sg, dst_sg, crypt_stat->key_size,
				default_key.data, default_key.len, crypt_stat->gcm_iv);
#else
		   ...
		   size_t *packet_size, size_t max_packet_size)
{
	...
	*packet_size += ECRYPTFS_SIG_SIZE;
	...
	(*new_auth_tok)->session_key.encrypted_key_size =
		(body_size - (ECRYPTFS_SIG_SIZE + 1 + 4 + 2 +
					  crypt_stat->pubkey_len + authsize + gcm_iv_len));
#else
	(*new_auth_tok)->session_key.encrypted_key_size =
